...
## Parameters used
n_folds = 6 # value of nfolds to use in data displayed in fit tab
train_dates = ['1/2/2005','1/2/2011']
test_dates = ['1/2/2011','1/6/2013']
...
for n in n_folds_list:
    ...
    if 'Unnamed: 0' in df_all_n.columns:
        df_all_n.set_index(df_all_n['Unnamed: 0'],inplace=True)
...
tab_fit = visu_tab_fit.fit_tab(agg,df_all,df_goal,candidates_data,train_dates,
    test_dates,old_to_new_sources,new_to_old_sources,old_to_new_regions)
tab_nfolds = visu_tab_nfolds.nfolds_tab(old_to_new_sources,old_to_new_regions,
                                   n_folds_list,agg_all_nfolds)
tab_map = visu_tab_map.map_tab(map_all_data,score_types_list,sources_list)
...
tabs = Tabs(tabs = [tab_map,tab_fit,tab_nfolds,tab_table,tab_tabledetails])
